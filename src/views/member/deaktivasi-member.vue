  ...
  export default defineComponent({  
    ...
    // Setup - add a text input to each footer cell
        ...
        async getAllMemberKadeluarsa(message){
            const url = "http://localhost:8000/api/showmemberdeaktif";
        ...
        async deaktivasi(message){
          const url = "http://localhost:8000/api/deaktivasimember";
            const request = await this.$http.get(url)
            this.members = request.data.data
            console.log(this.members)
            if(this.countInit == 0)
...
<template >
  <header>
    <home-navbar :message="'Gofit - Deaktivasi Member'"></home-navbar>
  </header>
  <main>
      <div class="text-dark table-custom mt-5 ms-5 me-5 p-2 d-inline-block">
        <button class="btn btn-primary" @click="deaktivasi">Deaktivasi Member</button>
      </div> 
            ...
            <table id="example" class="#example table table-striped filters" style="width:100%;">
                ...
                <tbody>
                    <tr v-for="(row,key) in members" :key="key">
                        ...
                        <td>{{row.tgl_kadeluarsa_aktivasi}}</td>
